...
def meshing_task(
    ...
    logging.info(f"Child {process.name} - Point cloud imported.")
    ...
    logging.info(f"Child {process.name} - Computing poisson mesh reconstruction...")
    ...
    logging.info(f"Child {process.name} - Processing completed.")
...
if __name__ == "__main__":
    ...
    for i, pcd in enumerate(pcd_list):
        ...
        if pcd_path.exists():
            logging.info(f"{pcd_path.name} already present. Skipping.")
            continue

        ret = meshing_task(pcd)
        if not ret:
            logging.warning(f"Error iter {i}, pcd {pcd}")
            continue
