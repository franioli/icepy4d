...
if __name__ == "__main__":
    ...
    def print_stats(x) -> None:
        ...
        print(
            f"AVG:\t{x[:, 0].mean():.2f}\t{x[:, 1].mean():.2f}\t{x.flatten().mean():.2f}"
        )
        print(
            f"STD:\t{x[:, 0].std():.2f}\t{x[:, 1].std():.2f}\t{x.flatten().std():.2f}"
        )
        print(
            f"MIN:\t{x[:, 0].min():.2f}\t{x[:, 1].min():.2f}\t{x.flatten().min():.2f}"
        )
        print(
            f"MAX:\t{x[:, 0].max():.2f}\t{x[:, 1].max():.2f}\t{x.flatten().max():.2f}"
        )
