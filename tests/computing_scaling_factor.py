...
# if epoch > 0:
#     for cam in cams:
#         cameras[epoch][cam] = cameras[cam][0]
...
for cam in cams:
    cameras[cam].append(
        Camera(
            width=im_width, height=im_height, calib_path=cfg.paths.caldir / f"{cam}.txt"
        )
    )
...
targets = [
    Targets(im_file_path=target_paths, obj_file_path="data/target_world_all.csv"),
]
...
for id, cam in enumerate(cams):
    im_coord[cam] = targets[0].extract_image_coor_by_label(targets_to_extract, id)
...
triangulation = Triangulate(
    [cameras[epoch][cams[0]], cameras[epoch][cams[1]]],
    [im_coord[cams[0]], im_coord[cams[1]]],
)
targets_local = triangulation.triangulate_two_views()

# Compute baselines and scale factor
baseline_local = np.linalg.norm(targets_local[0] - targets_local[1])
baseline_world = np.linalg.norm(targets_world[0] - targets_world[1])
(
    [cameras[epoch][cams[0]], cameras[epoch][cams[1]]],
    [im_coord[cams[0]], im_coord[cams[1]]],
)
